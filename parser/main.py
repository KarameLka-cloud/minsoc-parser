...
try:
    ...
    df = pd.read_excel(xls, sheet_name=sheet_name, skiprows=EXCEL_SKIP_ROWS)
...
log_ok(f"Таблица загружена      → {df.shape[0]} строк, {df.shape[1]} столбцов")
...
for _, row in df.iterrows():
    ...
    try:
        ...
        log_ok(f"Перенесён              → {file_name:<40} →  {folder_name}/")
        moved += 1
    except (shutil.Error, OSError) as e:
